...
void *DataUARTHandler::readIncomingData(void)
{
  ...
  catch (std::exception &e1)
  {
    ...
    catch (std::exception &e2)
    {
      ...
      ROS_ERROR(" => Port is \"%s\" and baud rate is %d", dataSerialPort, dataBaudRate);
      pthread_exit(NULL);
    }
  }
  ...
  while (ros::ok())
  {
    ...
    // ROS_INFO("DataUARTHandler Read Thread: last8bytes = %02x%02x %02x%02x %02x%02x %02x%02x", last8Bytes[7],
    // last8Bytes[6], last8Bytes[5], last8Bytes[4], last8Bytes[3], last8Bytes[2], last8Bytes[1], last8Bytes[0]);
...
void *DataUARTHandler::sortIncomingData(void)
{
  ...
  while (ros::ok())
  {
    ...
    case READ_HEADER:
      ...
      // make sure packet has at least first three fields (12 bytes) before we read them
    ...
    case READ_OBJ_STRUCT:
      ...
      i = 0;
      sdk_version = (mmwData.header.version >> 24) & 0xFF;
      // ROS_INFO_STREAM("*** sdk_version: " << sdk_version);
      if (sdk_version < 3)
      ...
      while (i < mmwData.numObjOut)
      {
        ...
        { // SDK version is older than 3.x
          ...
          temp[0] = static_cast<float>(mmwData.objOut.x);
          temp[1] = static_cast<float>(mmwData.objOut.y);
          temp[2] = static_cast<float>(mmwData.objOut.z);
          temp[3] = static_cast<float>(mmwData.objOut.dopplerIdx);
          ...
          temp[4] = static_cast<float>(mmwData.objOut.rangeIdx) * vrange;
          ...
          RScan->points[i].x = temp[1];   // ROS standard X-axis is forward => Y-axis of mmWave sensor
          RScan->points[i].y = -temp[0];  // ROS standard Y-axis is left => -(X-axis) of mmWave sensor
          RScan->points[i].z = temp[2];   // ROS standard Z-axis is up => Z-axis of mmWave sensor
        ...
        { // SDK version is 3.x+
          ...
          radarscan.intensity = static_cast<float>(mmwData.sideInfo.snr) / 10.0;
    ...
    case CHECK_TLV_TYPE:
      if (tlvCount++ >= mmwData.header.numTLVs)  // Done parsing all received TLV sections
      {
        ...
        // ROS_INFO("DataUARTHandler Sort Thread : CHECK_TLV_TYPE state says tlvCount max was reached,
...
void DataUARTHandler::visualize(const ti_mmwave_rospkg::RadarScan &msg)
{
  ...
  marker.color.a = 1;
  marker.color.r = static_cast<int>(255 * msg.intensity);
  marker.color.g = static_cast<int>(255 * msg.intensity);
  marker.color.b = 1;
